...
def test_top_fragments_zero_top_n():
...
def test_top_fragments_negative_top_n():
...
def test_top_fragments_empty_text_entries():
...
def test_top_fragments_none_text_entry():
    """Entry with None weight should be handled."""
    entries = [("hello world", None)]
    result = calculate_top_text_fragments(entries, top_n=5)
    # None weight should be treated as 0, then converted to 1
    assert len(result) > 0


def test_top_fragments_zero_weight():
...
def test_top_fragments_text_too_short():
...
def test_top_fragments_text_exactly_min_length():
...
def test_top_fragments_custom_min_length():
...
def test_top_fragments_preserves_display_case():
